...
- **認証機能 (AWS Cognito)**: メールアドレスベースのユーザー認証（オプション）
- **ユーザー分離機能**: 認証ユーザーはCognito ID、未認証ユーザーはブラウザIDで独立したデータ空間を提供
...
- **セクション分割生成**: AWS Amplifyタイムアウト対策として、セクション単位での段階的生成に対応
...
```
┌─────────────────────────────────────────────────────────────┐
│ Phase 1: コンテキスト準備 (1回のみ)                          │
│ /api/prepare-context                                        │
│ • RAG検索（CustomStakeholderQueryEnhancerによる5クエリ）     │
│ • S3ファイル取得                                            │
│ • GSNファイル取得                                           │
│ • 文書有効性チェック → 失敗時は即座にエラー                  │
│ • 戻り値: combinedContext + metadata                        │
└─────────────────────────────────────────────────────────────┘
                         ↓
┌─────────────────────────────────────────────────────────────┐
│ Phase 2: セクション生成 (セクション数分繰り返し)              │
│ /api/generate-section (×7)                                  │
│ • 事前取得済みコンテキストを使用                            │
│ • RAG検索なし、S3アクセスなし                               │
│ • Claude APIでセクション生成のみ                            │
└─────────────────────────────────────────────────────────────┘
```

#### メリット
- **タイムアウト回避**: 各APIリクエストが25秒以内に完了
- **RAG検索の効率化**: 7回→1回に削減（約86%削減）
- **早期エラー検出**: 文書がない場合、生成開始前にエラー表示
- **進捗表示**: セクションごとの生成進捗をリアルタイム表示
...
- **ログイン / ログアウト**: AWS Cognito認証（メール認証）
...
#### 概要
AWS Cognitoを使用したメールアドレスベースの認証機能を提供します。認証は**オプション**であり、未認証でも基本機能は利用可能です。

#### 認証状態による動作の違い
| 状態 | ユーザー識別子 | データ永続性 |
|------|--------------|-------------|
| **認証済み** | Cognito User ID | 永続的（どのブラウザからでもアクセス可能） |
| **未認証** | ブラウザID (UUID) | ブラウザ固有（localStorage依存） |

#### 認証フロー
1. 設定メニューから「ログイン」をクリック
2. メールアドレスとパスワードを入力
3. 新規ユーザーは「アカウント作成」→ メール認証コード入力
4. ログイン成功後、Cognito User IDでデータが管理される

### ユーザー分離機能

#### 概要
認証状態に関わらず、各ユーザーが独立したデータ空間を持つことができます。異なるユーザーが同じステークホルダーを選択しても、データが混在することはありません。

#### 仕組み
- **認証済みユーザー**: Cognito User ID (`cognito_xxxxxxxx`)
- **未認証ユーザー**: ブラウザID (`browser_xxxxxxxx-uuid-here`)
- Pineconeのネームスペースが `{stakeholderId}_{userIdentifier}` の形式で作成
- 例：認証済みユーザーが「CxO」を選択 → `cxo_cognito_abc123` のネームスペースを使用
...
#### 日本語フォント対応
日本語が正しく表示されるよう、以下の技術を使用：
- **@react-pdf/renderer**: Reactベースの高品質PDF生成
- **Google Fonts (Noto Sans JP)**: Webフォントによる日本語表示
- **テキスト正規化**: 日英混在テキストの改行処理を最適化
...
│   │   │   ├── generate-report/
│   │   │   │   └── route.ts          # レポート生成API（一括生成）
│   │   │   ├── prepare-context/
│   │   │   │   └── route.ts          # コンテキスト準備API（セクション分割用）
│   │   │   ├── generate-section/
│   │   │   │   └── route.ts          # セクション生成API（セクション分割用）
│   │   │   ├── export-html/
│   │   │   │   └── route.ts          # HTML出力API
│   │   │   ├── export-docx/
│   │   │   │   └── route.ts          # Word出力API
...
│   │
│   ├── hooks/
│   │   └── useSectionGeneration.ts  # セクション分割生成カスタムフック
│   │
│   ├── lib/
│   │   ├── config/
│   │   │   └── constants.ts            # アプリケーション設定値
│   │   ├── amplify-config.ts            # AWS Amplify/Cognito設定
...
- **対処法**:
  ...
  2. 新規ユーザーの場合、メール認証コードを入力したか確認
  3. Cognito User Poolの設定を確認

#### 認証後もデータが見えない
- **症状**: ログイン後、以前のデータにアクセスできない
- **原因**: 未認証時（ブラウザID）と認証時（Cognito ID）でネームスペースが異なる
- **対処法**: 
  - これは仕様です。認証状態によってデータ空間が分離されます
  - 必要に応じて、同じステークホルダーで知識ベースを再構築してください
...
- **AI/LLM**: 
  - Anthropic Claude API (Claude Sonnet 4) - レポート生成
...
- **認証**: AWS Cognitoによるセキュアな認証。パスワードはCognito側で管理され、アプリケーションには保存されません
- **ユーザー分離**: 認証済みユーザーはCognito User ID、未認証ユーザーはブラウザIDで完全に分離
- **データアクセス**: 各ユーザーは自身のネームスペースのデータにのみアクセス可能
- **Pinecone APIキー**: 環境変数で管理し、フロントエンドには露出させないでください
- **S3バケット**: 適切なIAM権限とバケットポリシーを設定してください
- **Cognito設定**: User Poolのセキュリティ設定（パスワードポリシー、MFA等）を適切に構成してください