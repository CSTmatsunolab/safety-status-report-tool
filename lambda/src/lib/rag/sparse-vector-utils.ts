...
async function getKuromojiTokenizer(): Promise<kuromoji.Tokenizer<kuromoji.IpadicFeatures>> {
  ...
  if (!initPromise) {
    ...
    const possibleDicPaths = [
      '/opt/nodejs/node_modules/kuromoji/dict',
      '/var/task/node_modules/kuromoji/dict',
      path.join(process.cwd(), 'node_modules', 'kuromoji', 'dict'),
    ];

    let dicPath = possibleDicPaths[possibleDicPaths.length - 1];
    for (const candidate of possibleDicPaths) {
      try {
        if (fs.existsSync(candidate)) {
          dicPath = candidate;
          console.log(`Found Kuromoji dict at: ${candidate}`);
          break;
        }
      } catch {
