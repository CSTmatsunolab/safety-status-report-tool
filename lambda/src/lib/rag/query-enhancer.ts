// src/lib/rag/query-enhancer.ts
// クエリ拡張モジュール - Next.jsと同等の実装
...
export class QueryEnhancer {
  ...
  enhanceQuery(
    ...
    if (prioritizedConcerns.length >= 3) {
      queries.push(
        ...
        prioritizedConcerns.slice(0, 2).join(' ')
      );
  ...
  protected generateEnglishQueries(stakeholder: Stakeholder): string[] {
    ...
    // GSN関連は常に追加
    queries.push('GSN Goal Strategy Evidence');
    
    // 安全性関連
    if (stakeholder.concerns.some(c => 
      c.includes('安全') || c.includes('リスク') || c.toLowerCase().includes('safety'))) {
      queries.push('safety risk mitigation');
    }
