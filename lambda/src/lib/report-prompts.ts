...
- \`[未完成の証拠連鎖 - 展開]\`: この役職の設定では本来まとめて要約する証拠連鎖だが、未展開項目・未達成・検証失敗を含むため例外的に展開されている。検証が完了していない事実と残作業を明示すること。
- \`◇ 内包する前提・文脈: ...\`という注記は、この見出しに内包され独立見出しにはしない前提（Assumption）・コンテキスト（Context）・正当化（Justification）である。**独立した見出しや章として起こしてはならない**が、この節の本文・要約の中で内容に必ず触れること。
