...
3. `lambda/src/index.ts` is the actual generation handler (streaming, SSE via `awslambda.streamifyResponse`). Its flow:
   ...
   - **Stakeholder-required sections**: a GSN-derived outline contains only GSN nodes, so role-specific decision material (CxO's basis for a go/no-go call, Product's release-decision info, …) has no heading and never gets written — and pass 2 may not add facts, so it can never be recovered later. `lambda/src/lib/stakeholder-requirements.ts` defines per-stakeholder required headings (title + what must be written + RAG queries; ja/en presets, keyword inference then a concerns-derived generic section for custom stakeholders). They are appended to the pass-1 outline (`appendRequiredSections` in `gsn/outline-generator.ts`, which reserves slots inside `MAX_SECTIONS` so they are never truncated), fed to `performGSNSubtreeAwareSearch` as `additionalQueries` (weight 1.1), described in the pass-1 structure prompt, and — in pass 2 — mapped *into the stakeholder's existing template headings* rather than added as new sections (`mapRequiredSectionsToTemplate`: each requirement carries a `placement` category — decision / risk / impact / technical / open-issues — resolved against the target structure's headings by keyword, with a fallback chain so nothing is dropped; the resulting "write this inside heading X" table plus a "never omit" rule goes into `restructure-prompts.ts`). The pass-2 target structure itself stays exactly `buildFinalReportStructure(...)` — the template's section list is never modified. Only applied when a GSN file is present — without GSN the static template already carries this material.
   - **Two-pass generation**: when the outline came from GSN (`outlineSource` is `hicase` or `gsn-flat`), the draft from pass 1 is fed back into Claude by `lib/restructure-prompts.ts` and rewritten into the stakeholder's configured section template (`buildFinalReportStructure(reportStructure, hasGSN)` — Executive Summary / Current State / Risk / Recommendations …). Pass 2 is a pure reorganization: the prompt forbids adding facts and forbids dropping Mandatory Safety Core items. The `progress` event with `status: 'restructuring'` (and any chunk) may carry `resetContent: true`, telling the client to discard the streamed draft and replace it with what follows; failure, or a suspiciously short pass-2 output, falls back to the pass-1 draft (re-sent as a `resetContent` chunk). Disable with `ENABLE_OUTLINE_RESTRUCTURE=false`. With no GSN file the outline is already the template, so pass 2 is skipped.
